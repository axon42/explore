...
for (const cutoff of [0, 120]) {
test(`audio capture with cutoff ${cutoff} requires consent and stops independently of the meeting`, async ({page}) => {
  let current: Record<string,unknown> | null = null;
  let starts = 0;
  const state = () => ({supported:true,helper_ready:true,configured:true,max_seconds:cutoff,capture:current});
  await page.route('**/api/audio-capture', route => route.fulfill({json:state()}));
  await page.route('**/api/sessions/*/audio-capture', route => {
    expect(route.request().postDataJSON()).toEqual({consent:true, microphone_participant_id:null, roster_revision:1});
    const sid = route.request().url().split('/').at(-2);
    starts++;
    current = {id:'capture-1',sid,status:'capturing',error:'',elapsed_seconds:cutoff ? 3 : 185,microphone_level:.2,system_level:.1,segments:0};
  ...
  if (cutoff === 0) await expect(dialog).toContainText('Capture continues until you select Stop capture or End interview.');
  else await expect(dialog).toContainText('Stops automatically after 2 minutes.');
  ...
  if (cutoff === 0) {
    await expect(page.locator('.ex-audio-meters')).toContainText('3:05 ·');
    await expect(page.locator('.ex-audio-meters')).not.toContainText('/ 2 min');
  }
  await page.screenshot({path:`/tmp/explore-audio-capture-${cutoff}.png`,fullPage:true});
