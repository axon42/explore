...
test("bottom-left analysis setting saves, follows other tabs and preserves interview controls", async ({ page, context, request }) => {
  ...
  await expect(page.getByLabel("Analysis mode", {exact:true})).toHaveAttribute("data-value", "legacy");
  const other = await context.newPage();
  await other.goto("/");
  await other.getByRole("button", {name:"Analysis settings",exact:true}).click();
  await selectOption(page.getByLabel("Analysis mode", {exact:true}), "topics");
  await expect(page.getByLabel("Analysis mode", {exact:true})).toHaveAttribute("data-value", "topics");
  await expect(other.getByLabel("Analysis mode", {exact:true})).toHaveAttribute("data-value", "topics");
...
test("stale reads cannot undo a saved mode and conflicts are visible", async ({ page }) => {
  ...
  await selectOption(page.getByLabel("Analysis mode", {exact:true}), "legacy");
