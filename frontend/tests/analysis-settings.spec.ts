...
test("bottom-left analysis setting saves, follows other tabs and preserves interview controls", async ({ page, context, request }) => {
  ...
  await request.post(`/api/workspaces/${workspace.id}/meetings`, { data: { title: "Settings interview" } });
  await page.goto("/");
  await page.getByLabel("Workspace", { exact: true }).selectOption(workspace.id);
  await expect(page.getByLabel("Analysis mode")).toHaveValue("legacy");
  const other = await context.newPage();
  await other.goto("/");
  await page.getByLabel("Analysis mode").selectOption("topics");
  await expect(page.getByLabel("Analysis mode")).toHaveValue("topics");
  await expect(other.getByLabel("Analysis mode")).toHaveValue("topics");
...
test("stale reads cannot undo a saved mode and conflicts are visible", async ({ page }) => {
  ...
  await page.getByLabel("Analysis mode").selectOption("legacy");
