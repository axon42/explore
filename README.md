# Still — local meeting transcripts

A minimal desktop-first React workspace backed by FastAPI and SQLite. Create a session, run a deterministic two-speaker demo, watch interim text become final, and reopen the saved transcript after a restart. No accounts, API keys, audio recording, LLMs, summaries, coaching, or Zoom integration.
...
After initial setup, **`bash scripts/dev.sh`** starts both servers. Open **http://127.0.0.1:5173**. Choose **New session**, optionally enter a title, then **Create session → Start demo → Stop session**. Ctrl-C stops both processes cleanly. Vite supports frontend hot reload; restart the command after backend/configuration changes.
